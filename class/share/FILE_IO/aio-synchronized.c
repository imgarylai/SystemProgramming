/* Program to Illustrate Asynchronous I/O with synchronized writes */
...
void main(int argc, char *argv[]) {
  ...
  if ((fd = open(argv[1], O_WRONLY | O_CREAT | O_TRUNC | O_SYNC, 0640)) == -1) {
    perror("Open failed: ");
    exit(1);
  }
  ...
  cb.aio_sigevent.sigev_notify = SIGEV_NONE; /* No signals */
  cb.aio_offset = 0;
  cb.aio_reqprio = 0;
